...
CLPG v0.1.0 is a declared-record implementation of the paper's local participation layer. It does not implement the full optimization theory. It checks whether the records required by that theory are present, internally consistent, and within declared policy thresholds.
...
v0.1.0 consumes declared certificates, digests, finite-set records, and bounds. It does not generate theorem-level certificates from the paper, validate upstream signatures, or prove that declared certificates are substantively correct.

| Paper concept | CLPG artifact | Strict check | v0.1.0 limitation |
|---|---|---|---|
...
| semantic roles | `ClaimRole` | role-aware terminal handling for `verify`, `withdraw`, `exit`, `assist`, `continue`, `act` | public `ParticipationDecision` keeps no separate `continue` value |
